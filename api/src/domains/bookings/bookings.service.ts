import type { CreateStaffBookingPayloadType } from "@/domains/bookings/bookings.validator.js";
import { BookingActor, BookingChannel, BookingStatus, GroundActions, GroundStatus, NotificationEvent, PermissionLevel, PitchStatus, SlotStatus } from "@/generated/prisma/enums.js";
...
export default class BookingService {
    ...
    private readonly buildPricingSnapshot = (ground: Ground, slots: Array<GroundSlot>) => {
        const pricingMap = {
    ...
    createStaffBooking = async (initiatorId: string, pitchId: string, groundId: string, payload: CreateStaffBookingPayloadType) => {
        ...
        if (payload.channel === BookingChannel.WALK_IN) { status = BookingStatus.CONFIRMED } 
            ...
            if (!paymentMethods.includes(payload.paymentMethod))
                throw new BadRequestError("Request booking must be paid for in one of the allowed payment methods.", ERROR_CODES.BOOKING_PAYMENT_METHOD_NOT_ALLOWED);
        ...
        const { pricingMap, pricingSnapshot } = this.buildPricingSnapshot(ground, slots);
