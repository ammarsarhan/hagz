    ...
    return (
        <>
            ...
            <InputGroup 
                ...
                value={data.company || ""} 
                onChange={(e: ChangeEvent<HTMLInputElement>) => setData({ ...data, company: e.target.value })}
            />
            <InputGroup 
                label="Email" 
    ...
    return (
        <>
            <InputGroup 
                ...
                placeholder="Street" 
                value={data.street} 
                onChange={(e: ChangeEvent<HTMLInputElement>) => setData({ ...data, street: e.target.value })}
            />
            <InputGroupContainer>
                <InputGroup 
                    label="District" 
                    ...
                    value={data.district} 
                    onChange={(e: ChangeEvent<HTMLInputElement>) => setData({ ...data, district: e.target.value })}
                />
                <InputGroup
                    label="City" 
                    placeholder="City" 
                    value={data.city} 
                    onChange={(e: ChangeEvent<HTMLInputElement>) => setData({ ...data, city: e.target.value })}
                />
            </InputGroupContainer>
            <InputGroup
                label="Governorate" 
                ...
                onChange={(e: ChangeEvent<HTMLInputElement>) => setData({ ...data, governorate: e.target.value })}
            />
            <InputGroup
                label="Country" 
                placeholder="Country" 
                value={data.country}
                onChange={() => setData({ ...data, country: "Egypt" })}
                readOnly
            />
        </>
    )
...
const Fourth = () => {
    ...
    const sendEmail = useCallback(async (email: string) => {
        ...
        const res = await fetch("http://localhost:3000/api/auth/owner/verify/send", {
            method: "POST",
            headers: {
...
const Form = () => {
    ...
    const createOwner = async () => {
        ...
        const res = await fetch("http://localhost:3000/api/auth/owner/sign-up", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Accept": ' application/json'
    ...
    const onSubmit = async (e: FormEvent) => {
        ...
        if (index == 2) {
            ...
            const auth = await signInWithCredentials(data.email, data.password, true);
    ...
    return (
        <form className={`flex-center flex-col h-full border-l-[1px] px-8 ${disabled ? "gap-y-8" : "gap-y-12"}`} onSubmit={onSubmit}>
...
export default function Signup() {
    ...
    const steps = [
        {
            ...
            description: "Create a new account to start creating pitches with Hagz!",
            component: <First/>,
            schema: z.object({
        ...
        },
        {
            ...
            description: "Provide a billing location we can reach you at. This can either be your own personal location, or a common workplace location.",
            component: <Third/>,
            schema: z.object({
                street: z.string({ message: "Street" }).min(5, { message: "Street name must include at least 5 characters." }),
                district: z.string({ message: "dis" }).min(3, { message: "District name must include at least 3 characters." }),
                city: z.string({ message: "city" }).min(3, { message: "City name must include at least 3 characters." }),
                governorate: z.string({ message: "gov" }).min(3, { message: "Governorate name must include at least 3 characters." }),
                country: z.string({ message: "count" }).min(3, { message: "Country name must include at least 3 characters." })
            })
        },
        {
    ...
    return (
        <div className="lg:grid grid-cols-2 h-screen">
            <div className="h-full hidden lg:block"></div>
